		...
		<label class="flex items-center justify-between cursor-pointer">
			...
			<div class="relative">
				<input 
					...
					class="sr-only"
				/>
				<div class="w-10 h-5 rounded-full shadow-inner transition-all duration-200 {sessionStore.autoFollow ? 'bg-blue-600' : 'bg-gray-600'}"></div>
	...
	<div class="space-y-2">
		<label class="flex items-center justify-between cursor-pointer">
			...
			<div class="relative">
				<input 
					type="checkbox" 
					checked={sessionStore.showBeatNumbers}
					onchange={() => sessionStore.toggleShowBeatNumbers()}
					class="sr-only"
				/>
				<div class="w-10 h-5 rounded-full shadow-inner transition-all duration-200 {sessionStore.showBeatNumbers ? 'bg-blue-600' : 'bg-gray-600'}"></div>
				<div class="absolute left-0.5 top-0.5 w-4 h-4 bg-white rounded-full shadow-md transform transition-all duration-200 {sessionStore.showBeatNumbers ? 'translate-x-5' : 'translate-x-0'}"></div>
			</div>
		</label>
		<p class="text-xs text-gray-400">
